...
### §D.33 JSM skills corpus intake (WP-F — the nine-skill invocation plane)
- **Canon:** WP-F build spec (`~/factory-rig/tmp/wp-f-jsm-merger/WP-F-build-spec.md`) — decision of record (operator session 2026-09-21): JSM's nine skills become the rig's **invocation plane**; the rig keeps the **enforcement plane** (bins, lint gates, `.tmd/` law, contracts) and the **discipline corpus**. Supersedes §D.30's stage-command-layer rejection **for this corpus only** (the agent-skills commands row stands) and the Design-stage ruling per WP-F §1 (lands with the /architect commit).
- **Source:** `github.com/jsmastery-pro/skills` @ `43b69e44c9ca905fe3a3418ccdf4102255e20d40` (v2.0.0, MIT, 2026-08-07). Intake clone: `sources/_intake/jsm-skills/` (machine shelf per §D.28 convention; SHA-verified at clone time). Adoption imports from the pinned clone, never a fresh fetch; acceptance for every JSM body = source diff where every hunk maps to an enumerated edit.
- **Includes:** nine skills — scope, audit, architect, develop, check, test, document, sync, debug — each landing as a MERGE: JSM body + rig incumbent content ported verbatim + enumerated rig bindings (per WP-F §4 merge maps; completeness-gated so no content truncates). Phase 2 order: audit → scope → architect → develop → check → test → document → sync → debug, one skill per commit.
- **Retirements (Phase 3, merge-completeness driver must exit 0 first):** spec-intake + slice-plan → /scope; pr-review → /check review (E.4/verified.md pipeline contract preserved); systematic-debugging → /debug **only after the trigger-parity check** (else it stays — deviation recorded, never silent dual-maintenance).
- **Status: Phase 2 LANDED 2026-09-21 (9/9 merges; one commit per skill)** — audit, scope (+spec-intake/slice-plan absorption), architect (+D-1 canon amendment), develop (+§1.2 subagent-posture amendment), check (+pr-review full port), test, document, sync, debug (+systematic-debugging absorption, parallel-run until trigger parity). All skills E.6-conformant, lint-skills VALID (35), skills-smoke 18/18, canon-register green. Phase 3 PENDING: completeness-gated retirements (spec-intake, slice-plan, pr-review; systematic-debugging after trigger parity) + §5 doc rewiring. Phases 4–5 PENDING (zcode-rig plugin, closeout).
...
- **Integration path:** Phase 2 merges → Phase 3 retirements → Phase 4 `zcode-rig` plugin (`projections/zcode-plugin/`, Package A Z.13 design ∪ WP-F §6; absorbs Z.12 open items V5/V6) → Phase 5 closeout (FACTORY_STATUS + V-joint closure rows).
...
### §D.34 pi-observational-memory (tiered observational memory extension — formal intake)
- **Canon:** WP-F open-item remediation (2026-09-21). The extension had been installed directly into the deployed clone (~/.pi/agent) by an earlier session — a read-only/pull-only violation that jammed the sync chain. Formal intake moves it into the authoring repo so its config arrives via commits; the deployed clone returns to pure pull-only.
- **Source:** local shelf `sources/_intake/pi-observational-memory` @ `78a1efcfdd46332253fb289724f05b26dfc7769e` (v0.1.0, MIT, no runtime deps). Byte-identical to the copy that was running in the deployed clone (diff-verified, node_modules/.git excluded).
- **Includes:** `extensions/pi-observational-memory/` (src, tests, agent/, configs); the `observational-memory` settings block in `settings.json` (verbatim from the deployed config: chunk 5k / pool 10k / consolidate 20k / tail 20k, passive: true, observer+consolidator on kimi-coding @ thinking low); runtime state (`seat-state.json`, `memory-state.json`, `models-store.json`) gitignored — machine-local, never committed (two-subdirectory rule §3.9 pattern).
- **Status: LIVE (intake landed 2026-09-21, WP-F remediation).**
- **Activation trigger:** NONE (running). **Review trigger:** first unexplained Kimi-seat token draw — the observer/consolidator workers run passively on every session; audit with `node ~/.pi/agent/bin/token-usage.mjs --session <id>`. Flip `passive: false` or remove the settings block only via rig-change.
- **Prerequisites:** Pi 0.84.3 extension API (verified live in the deployed clone since install); pin recorded in `package-pins.json`.
- **Integration path:** authoring repo → push → deployed pull (operator: discard the local settings.json mod — identical content arrives by commit; remove the untracked extension dir — identical content arrives tracked; runtime state files are ignored in place).
- **Reason class:** operator decision gate — governance remediation (read-only clone law restored) with config preserved verbatim; related: §D.32 (agent memory system, WP-MEM) — this extension is the Pi-side observation substrate, distinct from the rig's governed memory skill.
