...
- Install each selected skill through the rig's intake door (WP-F E4): on a governed machine, route each pick through the `tool-intake` skill (exact pin + integrity hash, `--ignore-scripts`, smoke fixture under `validation/`, register row via `rig-change`) — never an ad-hoc install. No rig installed: `npx skills add <owner>/<repo> -y` stands. Connecting an MCP is a user config step (their MCP or connector settings, e.g. `claude mcp add …`); on a governed machine the curated MCP stack is staged per `templates/mcp-catalog.json` via `tool-intake` + `rig-change`, not hand-connected; you can't do it for them, so point them there. Once connected the tools are used automatically.
