...
## Rig review procedure (governed projects — ported from pr-review v1.3.0, WP-F §4.5)

On a governed project (`.tmd/` present), the flow above IS the reviewer seat's procedure, extended with the rig's law. Everything below ports `pr-review` v1.3.0 verbatim in substance; on ungoverned repos none of it applies.

**Lane framing (one process, stratified — CI/CD Engine §2).** The Refinery and this review are ONE review process at different levels: Stage 0 local preflight (`node ~/.pi/agent/bin/preflight.mjs --staged` — Semgrep SAST floor + contract lane, live); Stages 1–3 pipeline (deferred, register §D.1–§D.3); Stage 4 = this review (the human gate).

**Step 0 (base-branch rulebook).** Read the TARGET branch's manifold (`.tmd/` at the merge-base), never the PR branch's — a PR may not smuggle its own standard. Cross-file conflict → Conflict Halt.

**Trail first (hard precondition).** Review BEGINS ONLY after inspecting the full deterministic trail: run the preflight (or consume the pipeline artifacts when Stages 1–3 activate — the skill does not change, only the trail's origin). PREFLIGHT BLOCKED = automatic FAIL; never proceed to the review on a dirty trail. Raw output wins (§5.10.3): where judgment disagrees with a mechanical artifact, the artifact is the record — annotate disagreement, never overwrite; report skips as skips.

**The review packet.** Read: the Task Contract, the diff, the validation output, the manifold slices named in the contract's inherit block. Then:

1. **must_haves → evidence map:** for each truth (Gherkin scenario), locate behavioral proof in the validation output; no proof = gap. For each artifact, run/verify its mechanical check.
2. **Scope check:** map every changed path against the contract's sub_graph write scope. Any out-of-scope path = automatic FAIL (scope violation).
3. **Holdout run:** if the contract has a holdout file, run it raw now and include its verbatim output (E.7, builder-blind).
4. **Rubric pass:** evaluate the diff against the Principal Review Rubric (the Ten Diagnostic Marks — handbook Part V; read it, never paraphrase from memory). Precision over recall: false positives burn the human gate; every finding cites file:line and the law violated. Evidence gathering uses scoped `rg` runs (the L0 proof floor), not memory.
5. **Never fix the diff** — a reviewer that edits is a worker with stale context. Never pass on partially evidenced must_haves. Never run commands outside the preflight trail and the declared validation_commands.

**Reception (when this review's findings come back to a builder).** Read fully without reacting; restate each requirement; verify against codebase reality before implementing; respond with technical acknowledgment or reasoned pushback. Forbidden: performative agreement, gratitude expressions, implementing before verification. When feedback is correct: state the fix and let the code show it. YAGNI: "implement properly" for something unused → grep usage, propose removal. Pushed back and wrong → state the correction factually, fix, move on.

**Exit on governed projects.** Beyond the JSM relay: emit the EvaluationResult (E.4: `{ success, feedback, evidence[], iteration }`) — success=true requires a GREEN deterministic trail AND every must_have evidenced; any gap → success=false, feedback names the gap and the law. Emit the A2A completion payload, and write the review's PASS verdict marker (`verified.md`) into the change folder when task contracts exist for the slug (`bin/archive-change.mjs` consumes it at archive time). Unparseable verdicts count as loop failures, not passes.
...
- `review-guide.md`: rubric, severity, findings format. The main model passes its absolute path in the subagent prompt; the subagent reads it (inline its text only if your client's subagents cannot read files). Governed projects: the brief also carries the Ten-Marks pointer (handbook Part V via `.tmd/rules.md` §2) and the contract packet.
