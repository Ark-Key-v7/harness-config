...
9. **Governed projects (rig rubric, WP-F port)**: when the spawn brief carries a `.tmd/` manifold, ALSO evaluate the diff against the Principal Review Rubric (the Ten Diagnostic Marks — read handbook Part V via the `.tmd/rules.md` §2 pointer; never paraphrase from memory), against the gravity Registry's boundaries, and against the contract's must_haves (every truth evidenced) and its holdout run output (verbatim in the findings). Every such finding cites file:line and the law violated; precision over recall — false positives burn the human gate.
