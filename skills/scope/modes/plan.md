...
Governed continuation (WP-F E1): on a governed project (`.tmd/` present), the plan answers compile into the rig's Phase-0 chain before the feature walk — offer it in the report and run it on confirmation: `modes/spec.md` authors `specs/intent/<slug>.md` + `specs/prd/<slug>.md` + `specs/changes/<slug>/delta.md` for the next slice (carrying forward what the interview already answered; lint-spec must exit 0), and after the row's decision lands (`/architect` where `needs a decision`), `modes/slices.md` decomposes the approved PRD into `specs/plans/<slug>.md` + task contracts (lint-contract green, REQ-ID threading intact). Roadmap rows and rig artifacts stay one truth: the roadmap is the index, the chain holds the detail.
