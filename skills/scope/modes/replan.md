...
5b. Governed continuation (WP-F E1): on a governed project, queue the next slice through the rig chain — `modes/spec.md` compiles the queued slice's intent/PRD/delta (skipping what shipped features' artifacts already answer), and once its decision is ratified, `modes/slices.md` cuts the plan + contracts. The roadmap row and the chain artifacts advance together.
