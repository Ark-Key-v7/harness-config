---
name: webperf-audit
description: Audits web application performance against Core Web Vitals. Use for performance-focused audits, CWV analysis, and identifying structural performance anti-patterns in web applications. Produces a severity-rated findings report with a metric-honest scorecard.
metadata:
  author: Agentic SWE Factory (ported: addyosmani/agent-skills, MIT)
  version: 1.0.0
  class: procedural
  trigger_phrases: ["performance audit", "audit CWV", "Core Web Vitals analysis", "webperf audit", "audit this page's performance"]
disable-model-invocation: true
provenance:
  source: addyosmani/agent-skills agents/web-performance-auditor.md
  imported: 2026-09-12
  transform: agent-to-skill; framing only, body verbatim
  edits: [frontmatter per spec §4.3, persona line to Overview, Rule 8/9 cross-skill paths, Composition to Act/Observe/Exit, metadata.class+trigger_phrases per IMPORT MODE step 4, When NOT to Use per IMPORT MODE step 5]
---
...
Audit a web application for performance bottlenecks, assess real-world user impact, and recommend concrete fixes. Findings are prioritized by actual or likely effect on Core Web Vitals and user experience.

## When NOT to Use

- Utility libraries, CLI tools, or non-web projects — Core Web Vitals do not apply (an audit would produce noise, not signal).
- Remediation/implementation work — this skill emits the audit report; optimization guidance lives in `../performance-optimization/SKILL.md`.
- When measured numbers are required but no Deep-mode artifacts exist — Quick mode findings are `potential impact` only; do not let a caller treat them as measurements.
...
8. Use `../performance-optimization/references/performance-checklist.md` as the minimum baseline for each area.
9. Delegate granular optimization guidance and remediation steps to `../performance-optimization/SKILL.md` — keep this report at the audit level.
...
## Act / Observe / Exit
- **ACT:** run the audit in Quick mode (source scan) unless tool artifacts or live measurement are available, in which case run Deep mode. Follow Review Scope sections 1–4.
- **OBSERVE:** populate the Output Format template. The scorecard is `not measured` unless values are backed by a named artifact.
- **EXIT:** emit the report. If Critical or High findings exist, the QA gate for this slice FAILS (needs_human) until fixed or explicitly waived by the operator.
