---
name: context-budget
description: Enforces L8 context budget discipline. Use when starting a new session, when agent output quality degrades, when switching between tasks, when trimming context at the 75% threshold, or when configuring rules files and context for a project.
metadata:
  author: Agentic SWE Factory (ported: addyosmani/agent-skills, MIT)
  version: 1.0.0
  class: discipline
  trigger_phrases: ["new session", "output quality degraded", "trim context", "switching tasks", "context setup"]
disable-model-invocation: true
provenance:
  source: addyosmani/agent-skills skills/context-engineering
  imported: 2026-09-12
  edits: [E1 frontmatter+name, E2 rules-file mapping, E3 MCP table, E4 metadata.class+trigger_phrases per IMPORT MODE step 4, E5 When NOT to Use per IMPORT MODE step 5]
---
...
- During a governed project's Phase-0/contract flow where `.tmd/` law already answers the question — read the manifold, do not curate a parallel context layer.
- As a substitute for the rig's own session/state machinery (`STATE.md` handoffs, fresh-context roster law) — this skill governs what goes in the window, not how seats are spawned.
...
In the rig, Level 1 is already occupied by law: the `.tmd/` manifold (rules/gravity/promises/glossary/design) and `AGENTS.md` at repo root. Do NOT create a CLAUDE.md-style parallel rules file — that would violate the Instruction-File Boundary (one law plane, referenced never duplicated). What this level means in practice:

- Persistent, project-wide law lives in `.tmd/` — maintained by project-onboard and rig-change, not by hand per session.
- The per-project equivalent of a "rules file" (tech stack, commands, conventions, boundaries) is the **Project PRD + `.tmd/design/`** produced by the Phase-0 chain. If you find yourself wanting to write a rules file, the correct move is to check whether project-onboard has run, not to author new prose.
- `AGENTS.md` is the portability surface: it points at the law plane for any harness that reads it.
...
| **GitHub** | Issue, PR, and repository context | Via `gh` CLI (machine floor); no MCP needed |
