...
This skill needs a live, responsive user. **Do not invoke in non-interactive contexts** like CI pipelines, scheduled runs, or headless operation. Per Harness §5.8 (headless ask-user ruling): if invoked headless, terminate as `needs_human` with a proposed answer instead of guessing.
...
If the user wants the intent to persist (a multi-session project, a handoff to another collaborator), save it to `specs/intent.md` per TCE §2.A Phase 0 (provenance header required). Only save if they confirm.
...
- **`spec-intake` divergent step**: downstream. If the confirmed intent is "I want X but I don't know how to scope it," hand off to `spec-intake`'s divergent refinement step to generate variations against the now-explicit intent.
- **`spec-intake`**: downstream. If the confirmed intent is concrete ("I want X for Y users with Z success criteria"), hand off to `spec-intake` to write it down.
- **`slice-plan`**: two hops downstream of this skill (after the spec).
- **`pr-review`**: opposite end of the timeline. Interview-me is pre-decision intent extraction; pr-review is post-decision artifact review. Both catch divergence, but at different moments.
