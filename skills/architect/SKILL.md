...
metadata:
  ...
  class: procedural
  trigger_phrases: ["architect", "design this feature", "decide the stack", "pick a database", "design the data model", "we need a decision", "architect the auth", "ratify the assumption"]
  version: 1.0.0
  provenance:
    source: jsmastery-pro/skills (skills/architect @ 43b69e44c9ca905fe3a3418ccdf4102255e20d40)
    method: "byte-copy + enumerated edits; acceptance = source diff"
    edits:
      - "E1 (SKILL.md, Rig bindings): governed-project decision landing — decisions ALSO record as ADRs (docs/decisions/, documentation-and-adrs lifecycle) and boundary decisions draft gravity §C.3 Sub-Graph rows via the Amendment Protocol (agent drafts, operator ratifies); $SPEC_DIR maps to specs/specs/ on governed projects, docs/specs/ elsewhere"
      - "E2 (SKILL.md, Rig bindings): AC-N + value-sourcing + build plan thread into task contracts (AC-N ↔ must_haves) via /scope slices mode"
      - "E3 (SKILL.md, Rig bindings): Assumed decisions map to contract assumption rows; only /architect clears them (JSM rule kept)"
      - "E4 (SKILL.md, Rig bindings): cross-check critic 'Another model' = a different provider family (rig seats: GLM ↔ Kimi ↔ configured OAuth); model pinning lives in agent definitions, never skills"
      - "E1 canon (CANON_MAP §1 Observation 1, this commit): Design-stage ruling superseded — /architect owns decision-authorship; /scope owns decomposition"
    additions:
...
Writes no code. Never updates `AGENTS.md`/`CLAUDE.md` (/sync owns that). Never edits `.tmd/` law — boundary decisions are DRAFTED as gravity Registry proposals for the operator to ratify through the Amendment Protocol.
...
- **Resolve the spec location** (`SPEC_DIR`) = the scope workspace mirrored into `docs/specs/`: single repo → `docs/specs/`; monorepo workspace → `docs/specs/<workspace>/`; repo wide → `docs/specs/_root/`. Determine `<workspace>` as the scope does (topic/path/scope row). Create the directory if missing. Governed project: see Rig bindings — decisions additionally land as ADRs and gravity drafts.
...
- **Read project context**, the source of truth for the stack and community skills: root `AGENTS.md` (fall back to `CLAUDE.md`, else MISSING), plus the nested `<area>/AGENTS.md` for this feature's area if one exists (e.g. `src/auth/AGENTS.md` for an auth feature). Governed project: the `.tmd/` manifold is the deeper source — gravity.md's Registry and rules.md bind any decision this feature touches; read the relevant sections before proposing options.
...
- The decision is already deliberated and recorded in an ADR or spec that still holds → nothing to architect; reference it. Re-run only when the decision is genuinely overturned (then supersede).
- The topic is a full product ("build me a SaaS that…") → the design conversation's Check A routes it to one foundational decision at a time; do not write a product-sized spec.
- Implementation questions ("how do I code X in framework Y") → `/develop` or documentation lookup; this skill decides, it does not teach syntax.
- Gravity/boundary changes on a governed project that the operator has not requested → boundaries are operator-authored law; architect drafts proposals only when a decision legitimately requires one, through the Amendment Protocol.

## Rig bindings (WP-F §4.3)

- **Governed decision landing (E1).** On a governed project (`.tmd/` present), `$SPEC_DIR` resolves to `specs/specs/` (the contract-prep artifact chain), and each confirmed decision ALSO lands as an ADR under `docs/decisions/` (the `documentation-and-adrs` lifecycle: PROPOSED → ACCEPTED → SUPERSEDED — this skill owns the content; the lifecycle is shared law). A decision that draws a new code boundary drafts a `gravity.md §C.3` Sub-Graph Registry row as a PROPOSAL: the agent drafts, the operator ratifies through the Amendment Protocol — never self-merged.
- **Contract threading (E2).** The spec's numbered ACs, the Value sourcing table, and the `## Build plan` are what `/scope`'s slices mode threads into task contracts (AC-N ↔ contract must_haves; value rows ↔ the build-time owed-decision gate). Write them so that threading is mechanical: every AC IDed, every value sourced, every task tagged.
- **Assumed ↔ contract assumption rows (E3).** `/develop` records an override as an assumption row in the contract; this skill ratifies or supersedes it exactly as for JSM `Assumed` specs. Only `/architect` clears an assumption — in the contract row, the ADR, and the spec status line together.
- **Cross-model critic (E4).** `Another model` means a capable model from a DIFFERENT provider family than the one that wrote the spec (rig seats: GLM ↔ Kimi ↔ any configured OAuth family). Pin models in agent definitions, never here; this file speaks capability-first.
- **Law is untouchable.** Never edit `.tmd/` files. Boundary proposals go through the operator, via the Amendment Protocol.

## Procedural form (ACT → OBSERVE → EXIT)

- **ACT** — Step 0 topic check; pre-flight (freshness, `$SPEC_DIR`, numbering, related specs, build approach, scope row, context files); read `internal/design-conversation.md` and run the staged conversation (Scope validation → Framing → stages a–f → completeness gate); read `agent-prompt.md` + `spec-template.md` + the one mode file and write the spec.
- **OBSERVE** — read `internal/after-subagent.md`: verify the write landed (file exists, no blank required sections — `⚠️ Incomplete` otherwise), offer the cross-check critic per tier, present the confirmation panel and loop until Accept; on Accept, ratify status per spec kind, derive tasks + link the scope row, and on governed projects land the ADR + gravity draft per the rig bindings.
- **EXIT** — spoken summary (decision, why, next step: `/clear` then `/develop <feature>` or `/scope` slices mode on governed projects). This skill writes no code, never commits, and never advances a feature-linked spec's status — `/develop` owns that line.
