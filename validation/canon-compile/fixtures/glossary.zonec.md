...
This file's project bindings — system-identifier bindings (factory-reserved entities are Zone A.4, fixed) — are authored by the
...
Canonical slot order: §B.1 System identifiers. A correct identifier entry
fills every table column — Identifier, Canonical, Forbidden Variants, Routing
Class (or `—`), As of (YYYY-MM). A definition you cannot apply as a decision
