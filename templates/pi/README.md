...
| `.mcp.json` | Curated, pinned MCP servers only; **empty is valid** (v1.2 §2.11) | Validated by `tools/lint-mcp.mjs` |
...
- `scope.json` — generated per Task Contract by
  `node ~/factory-rig/sources/harness-config/tools/contract-scope.mjs --contract <task.md> --gravity .tmd/gravity.md --out .pi/scope.json`.
...
6. Every server entry is exact-pinned (`name@x.y.z` — never `latest`, never
   a bare name) and must pass `node tools/lint-mcp.mjs .pi/.mcp.json`.
