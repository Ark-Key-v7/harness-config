...
### A.2 The fifteen global negative constraints (canon, fixed)
...
11. **NO CORPSE CODE:** NEVER comment out code to disable it. NEVER leave unreachable branches behind a flag you will "remove later." Delete it or gate it behind a declared kill switch. Version control remembers; commented code rots.
12. **NO CLONE-AND-OWN:** NEVER duplicate a block to vary it. Extract and parameterize; the second copy is the defect.
13. **NO UNBOUNDED LOOPS:** NEVER write a loop, recursion, pagination walk, or retry sequence without an explicit bound and an exit. Long-lived processes (servers, daemons, consumers) are declared in gravity.md §6 with a named brake — everything else terminates by count.
14. **NO SLEEP-BASED WAITS:** NEVER synchronize on wall-clock time. Wait on events, conditions, or explicit state transitions. Fixed sleeps are permitted only inside declared timing utilities, which themselves obey NO UNBOUNDED LOOPS.
15. **NO UNVALIDATED MODEL OUTPUT:** NEVER branch control flow on free-text LLM output without schema validation. Parse through a declared schema at the trust boundary; unparseable output is an error, routed topologically — never a guess.
...
This file's project bindings — scoped-law `applies_to:` declarations — are authored by the
...
**Law Economy (canon §0.7).** The global budget is fifteen laws — full. A
sixteenth global law is forbidden. New constraints descend the placement
ladder: path-scoped law here (applies_to:) → Zone C PRD-compiled entry →
skill-local constraint. Promotion up the ladder requires trace-ledger evidence
that the narrower placement failed. The budget counts laws, not words:
splitting one law into two to evade the budget is itself a violation.
Demotion path: global → scoped → Zone C → skill-local → removed.
...
### §C.1 PRD-compiled negative constraints
<!-- TEMPLATE_VALUE_REQUIRED — entries compiled from the PRD's requirements
     table (template/specs/prd.md "compiles into rules.md Zone C" rows).
     Micro-example of a completed block:
```yaml
# - constraint: "pagination in <slice> capped at N pages (Law 13)"
#   derived_from: specs/prd/booking.md#R4
