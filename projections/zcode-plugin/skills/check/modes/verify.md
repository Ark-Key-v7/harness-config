...
Governed project (`.tmd/` present): also load the governing **task contract's `must_haves`** (Gherkin truths + artifacts) into the same checklist — each truth is an observable behavior, each artifact a mechanical check, and the contract's `tier:` field sets how much of this tail the slice owes (WP-F D-2; the floor — preflight + verification discipline — is never tier-conditional). Holdout truths (E.7) are NOT run here — they are builder-blind and belong to `/check review`.
