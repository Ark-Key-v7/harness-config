...
metadata:
  ...
  class: procedural
  trigger_phrases: ["sync", "sync the docs", "reconcile after merge", "update the context files", "close the loop on this change"]
  version: 1.0.0
  provenance:
    source: jsmastery-pro/skills (skills/sync @ 43b69e44c9ca905fe3a3418ccdf4102255e20d40)
    method: "byte-copy + enumerated edits; acceptance = source diff"
    edits:
      - "E1 (SKILL.md, Step 3 + Rig bindings): governed orchestration — runs bin/archive-change.mjs (verified deltas → living domain specs) and rules-drift-check as steps; roadmap statuses reconciled"
      - "E2 (SKILL.md, Rig bindings): descriptions-only law — AGENTS.md content is descriptions + pointers; .tmd/ is law, never synced"
      - "E3 (SKILL.md, Rig bindings): Assumed/assumption-row surfacing preserved; archive-change's verified.md gate respected (never run on an unverified change)"
      - "E4 (SKILL.md, Step 2.5): installs route through tool-intake on a governed machine"
    additions:
...
- Find all specs under `docs/specs/` whose names start with a digit, sorted. Governed project: also locate the change's rig artifacts (`specs/changes/<slug>/` with its delta and contracts; `specs/domains/` living specs; `specs/roadmap.md` if present).
...
- Install selected skills through the rig's intake door (WP-F E4): on a governed machine, route each pick through `tool-intake` (pins + integrity + smoke fixture + register row); raw `npx skills add <owner>/<repo> -y` stands only without the rig. For MCPs, point to the user's connector/MCP settings (governed: the curated stack stages via `tool-intake` + `rig-change` per `templates/mcp-catalog.json`); once connected the tools are used automatically.
...
Governed continuation (WP-F E1), after the JSM maintenance completes and only when a rig change folder exists for the change:
- **Archive the change:** run `node ~/.pi/agent/bin/archive-change.mjs --change specs/changes/<slug>` — the deterministic merger (fail-closed preflight; consumes the reviewer's `verified.md` PASS marker when task contracts exist). An unverified change (no E.4 PASS) is not archived: surface it as decision debt, never force the merge.
- **Rules drift:** run `rules-drift-check` as a step and relay its advisory report (it stays advisory — apply edits only when the operator explicitly asks).
- **Roadmap:** reconcile `specs/roadmap.md` statuses from evidence, exactly under the scope Boundaries above (never downgrade the operator's `done`).
...
- Archive pending (governed): `specs/changes/<slug>` (<unverified — needs /check review PASS, or merged clean>)
```
...
- Nothing source-side changed (docs/tests/lockfiles only) → `NOTHING_TO_SYNC` is the honest exit; do not invent edits.
- The repo needs its context bootstrapped or a whole undocumented area documented → `/audit` owns that; /sync maintains, it does not bootstrap.
- A spec needs its content changed or superseded → `/architect` owns spec content; sync flags staleness, never rewrites.
- The operator wants the roadmap re-planned or features re-ordered → `/scope` replan; sync reconciles statuses from evidence only.

## Rig bindings (WP-F §4.8)

- **Descriptions only (E2).** `AGENTS.md` content is descriptions and pointers; law lives exclusively in `.tmd/` — never synced, never restated, never edited here. A manifold/code conflict is flagged for the operator (Conflict Halt posture), never resolved by edit.
- **Governed orchestration (E1).** `/sync` is the wrapper the operator runs instead of remembering bins: archive-change (deterministic spec merge), rules-drift-check (advisory), roadmap reconciliation. The skill points; the bins guarantee. `archive-change` refuses an unverified change by design — surfacing that refusal is success, not failure.
- **Assumed / assumption rows (E3).** Surfaced as decision debt every time, reconciled never; only `/architect` clears them (contract rows and `Assumed` specs together).
- **Idempotency law kept.** Run twice on the same change, zero new edits the second time. "When in doubt, flag rather than write"; default to doing nothing.

## Procedural form (ACT → OBSERVE → EXIT)

- **ACT** — scope the change set (name-status diff + untracked), filter to sources, locate targets (paths only), run tool discovery (consent gate first), then the maintenance per `agent-prompt.md`; governed: archive-change + rules-drift-check + roadmap reconciliation.
- **OBSERVE** — verify the edits match the Boundaries table exactly (status lines only, net-new-area creation only, no curated rewrites); idempotency self-check (a second run edits nothing); every heuristic edit listed for review.
- **EXIT** — relay the summary (reconciled line + Heads up), surface decision debt and archive-pending states, advise `/clear`. Never commits; never runs `/architect` or `/audit` for you — it points, you decide.
