...
description: "Designs stable API contracts and interfaces. Use when a slice adds or changes an endpoint, event schema, or public interface. Discipline class — binds to the worker seat; also the reviewer's contract-conformance lens for interface diffs."
metadata:
  author: Agentic SWE Factory
  trigger_phrases: ["design the API", "new endpoint", "event schema", "public interface", "API contract"]
  class: discipline
  version: 1.0.0
...
- This discipline is how the worker seat satisfies the scoped law EXPLICIT_AUTH_PER_ENDPOINT — every endpoint's auth requirement is declared in the contract, never implied.
- Idempotency keys, error shapes, and versioning decisions that affect the public contract are Zone-C-compilable law: when a project ratifies them, they land in `.tmd/rules.md` via the Canon Compiler path, and this skill's defaults yield to the project's compiled law.
- Contract changes are diffs the reviewer seat checks for addition-over-modification; breaking changes without a ratified ADR (documentation-and-adrs) fail review.

**When NOT to Use:** internal-only refactors with no public-contract change, or exploratory spikes whose interfaces will not survive.
