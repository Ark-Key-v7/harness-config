...
metadata:
  ...
  provenance:
    ...
    edits:
      - "E1 (SKILL.md, Rig bindings): ship-gate interface — a governed PR description leads with deterministic evidence; /document produces that lead from the gate trail and the body prose follows it"
      - "E2 (SKILL.md, Rig bindings): ADR touchpoint — a diff that lands a decision points to /architect's ADR (documentation-and-adrs owns ADR content law); /document references, never authors ADRs"
    additions:
...
- **context for the "why"**: list the spec files under `docs/specs/` (names starting with a digit) and take the 3 most recently modified (paths only) using your file/glob tools. Governed project: also list the change's rig artifacts (`specs/changes/<slug>/`, the task contracts, the review findings in `docs/reviews/`) — they are the evidence trail.
...
- ADR authoring or revision — `documentation-and-adrs` owns ADR content law; /document references an ADR in prose, it never writes one.
- Scope/spec/spec-chain authoring — `/scope` and `/architect` own those artifacts; /document writes human-facing prose about shipped work.
- Pre-diff narratives ("write the PR for what I plan to do") — everything here traces to the real diff; there is nothing to write before the change exists.

## Rig bindings (WP-F §4.7)

- **Ship-gate interface (E1).** On a governed project the PR description leads with deterministic evidence (ship-gate law): /document builds that lead from the change's gate trail — preflight result, gate driver output, review verdict, holdout run — and the narrative follows it. A narrative-only PR body is returned by ship-gate; do not write one.
- **ADR touchpoint (E2).** If the diff lands a decision (an ADR-worthy architecture or contract change), /document links the ADR in the prose and flags its absence as a Heads up ("decision without an ADR — `/architect` owns it"), never authoring ADR content itself.
- **Evidence trail (governed).** Prefer the change's rig artifacts as sources: `specs/changes/<slug>/delta.md`, the review findings (`docs/reviews/`), the E.4 verdict, `verify.md` ticks. "The diff wins" over every other account, including these.
- **Law is untouchable.** Never edit `.tmd/`, specs, contracts, or code; commits/merges belong to the operator and ship-gate.

## Procedural form (ACT → OBSERVE → EXIT)

- **ACT** — determine the type (argument, inference + confirm, or the panel), gather the source material (git/gh checks, evidence trail, incident facts for postmortems), read `agent-prompt.md` + the one template, write the document.
- **OBSERVE** — verify every sentence traces to the record (commits, diff, incident facts, gate trail); confirm the output landed where the type says (file written / body shown); changelog idempotency checked.
- **EXIT** — relay the result block (PR body in full for `pr`), tick the scope's `Document it` (closing gate), state what was not committed. Never commits, pushes, or merges; never writes code, tests, specs, or ADRs.
