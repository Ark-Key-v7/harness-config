...
description: "Instruments code so production behavior is visible: structured logs, RED/USE metrics, traces, symptom-based alerts. Use when building any feature that runs in production, adding I/O, retries, queues, or cross-service calls. Discipline class — binds to the worker seat."
metadata:
  author: Agentic SWE Factory
  trigger_phrases: ["instrument this", "add telemetry", "structured logs", "metrics", "alerts"]
  class: discipline
  version: 1.0.0
  provenance: "agent-skills @ 48cb1168 (2026-09-06), WP-D-6 adoption; verbatim + edits E1-E4"
---
...
- Telemetry written under this discipline is the substrate the deferred register presumes: the trace ledger (§D.15), production loop closure (§D.20), and the error-budget gate in ship-gate all consume signals this skill creates. A slice whose must_haves include production behavior and whose diff adds zero telemetry fails the reviewer seat's conformance check.
- Correlation IDs and entry-point stamping are mandatory law here, not style: they are what make tripwire and ledger events attributable.

**When NOT to Use:** throwaway scripts, test fixtures, and code that never runs in production.
