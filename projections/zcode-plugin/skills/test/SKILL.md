...
metadata:
  ...
  class: procedural
  trigger_phrases: ["test", "write tests", "test the change", "cover the diff with tests", "lock in the tests"]
  version: 1.0.0
  provenance:
    source: jsmastery-pro/skills (skills/test @ 43b69e44c9ca905fe3a3418ccdf4102255e20d40)
    method: "byte-copy + enumerated edits; acceptance = source diff"
    edits:
      - "E1 (SKILL.md, Rig bindings): TDD precedence — the rig's test-driven-development Iron Law (no production code without a failing test) governs the build; /test is the completion pass that locks durable coverage; it never contradicts red-green evidence already in the branch"
      - "E2 (SKILL.md, Rig bindings): traceability bridge — AC-N tags cross-reference contract must_haves IDs and the REQ domain requirements manifest on governed projects"
      - "E3 (SKILL.md, Rig bindings): GATE_ONLY maps to the rig gate ladder (check:fast) when one exists, else typecheck+verify"
    additions:
...
- **`tool` is `null` and `gate` is set** (`GATE_ONLY`): this project gates without a test runner, by an earlier deliberate choice. Do not write a suite, do not install a runner, do not ask again, and do not read `modes/setup.md`. Run the project's typecheck/lint gate (governed project: `check:fast` is the gate when a rig gate ladder exists, per Rig bindings), then stop and report: "This project gates on `<gate>`, not a test suite. Ran the gate; use `/check verify` to confirm behavior."
...
- Production code is about to be written with no test yet — that is `test-driven-development`'s Iron Law territory (failing test first); /test is the completion pass, not a substitute for red-green.
- A failing test during the build → `systematic-debugging` / the `/debug` skill investigates; /test writes coverage, it does not root-cause.
- The change is throwaway at the `Prototype` tier with the gate-only choice recorded — run the gate (`GATE_ONLY`), skip the suite.
- Verifying behavior by running the app → `/check verify`; /test writes the assertions that run forever.

## Rig bindings (WP-F §4.6)

- **TDD precedence (E1).** The `test-driven-development` discipline governs the build itself (no production code without a failing test; exceptions are the operator's call). /test assumes red-green evidence already exists in the branch and completes the suite around it: sweep the diff for uncovered edges and error states, never weaken or duplicate a red-green proof, and treat a failing test that exposes real broken code as a finding (BUGS_FOUND), never something to silence.
- **Traceability bridge (E2).** `AC-N` tags cross-reference the contract's `must_haves` IDs and the delta's `REQ-<domain>-<nnn>` requirements (the slice's `requirements:` manifest) on governed projects, so suite ↔ contract ↔ delta trace in one hop.
- **Gate ladder (E3).** `GATE_ONLY` runs the rig's `check:fast` gate when a gate ladder exists in the project; otherwise typecheck + `/check verify` as JSM defines it.
- **Law is untouchable.** Never edit `.tmd/`; never modify application code to make a test pass; commits follow `flow/git.md`-style offers, never push.

## Procedural form (ACT → OBSERVE → EXIT)

- **ACT** — scope from git, classify files, load preferences (setup on first run), installation check, gather pointers, write the suite per `agent-prompt.md` + `writing-guide.md`.
- **OBSERVE** — run the suite when `RUN_AFTER=yes`; verify pass/fail counts are real (never report a run that did not happen); map AC-N coverage and record NOT_COVERED honestly; flag BUGS_FOUND.
- **EXIT** — tick the scope's `Test it` only on a real pass, offer `done` (never gate it), mirror the spec status, relay the report block, save preferences state. Never commits; never invokes other skills.
